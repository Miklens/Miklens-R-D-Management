...
const logSchema = z.object({
  ...
  workType: z.enum(['project', 'company']),
  // Main fields
        ...
        <div className="lg:col-span-2">
          ...
          >
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              {/* Product/Experiment / Formulation / Trial Linking */}
              <div className="p-4 rounded-2xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 space-y-4">
                <p className="text-xs font-bold text-emerald-800 dark:text-emerald-300 uppercase tracking-wider">Product & Experiment Association</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      {...register('productId')}
                      className="w-full px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 font-medium"
                    >
                      <option value="p1">BioShield Alpha (Bio-fungicide)</option>
                      <option value="p2">NemaKill Pro (Bio-nematicide)</option>
                      <option value="p3">RootBoost X (Bio-stimulant)</option>
                      <option value="p4">AeroSpore V2 (Bio-insecticide)</option>
                      <option value="p5">Pseudomonas SC (Bio-control)</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                      Trial / Experiment Type
                    </label>
                    <select
                      {...register('experimentId')}
                      className="w-full px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 font-medium"
                    >
                      <option value="e1">Lab Efficacy Check (Phase 1)</option>
                      <option value="e2">CIPAC Stability Test (54°C Heat)</option>
                      <option value="e3">On-Farm Field Trial (Plot A)</option>
                      <option value="e4">Trial & Error Formulation Batch</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                  Trial Task Preset <span className="text-gray-400">(1-click fill)</span>
                </label>
                <div className="flex flex-wrap gap-2 mb-3">
                  {[
                    { label: '🧪 Lab Efficacy Check', text: 'Checked microbial suspension & CFU count in lab' },
                    { label: '🔥 54°C Heat Stability Test', text: 'Accelerated heat stability check at 54°C for 14 days' },
                    { label: '🌾 Field Spray Application', text: 'Applied foliar spray trial in Plot 4 with control group' },
                    { label: '⚗️ Surfactant Mix Trial', text: 'Formulation trial adjusting wetting agent & dispersion rate' },
                  ].map((preset) => (
                    <button
                      key={preset.label}
                      type="button"
                      onClick={() => {
                        setValue('objective', preset.label);
                        setValue('activities', preset.text);
                      }}
                      className="px-3 py-1.5 bg-gray-100 dark:bg-gray-800 hover:bg-emerald-100 dark:hover:bg-emerald-950/40 text-gray-700 dark:text-gray-300 hover:text-emerald-700 dark:hover:text-emerald-300 rounded-lg text-xs font-semibold border border-gray-200 dark:border-gray-700 transition-all"
                    >
                      {preset.label}
                    </button>
                  ))}
                ...
                <input
                  type="text"
                  {...register('objective')}
                  placeholder="Brief title or task summary..."
                  className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500"
                />
                {errors.objective && (
